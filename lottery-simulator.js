 ...
 */
function getHeroPoolForBucketWithExtra(bucketString, bucketIndex, poolConfig, masterHeroPool) {
    console.log(`[getHeroPoolForBucketWithExtra] 调用: ${bucketString}, index: ${bucketIndex}, masterPool长度: ${masterHeroPool.length}`);
    const extraRules = poolConfig._extraBucketConfigs ? poolConfig._extraBucketConfigs.find(rule => rule.index === bucketIndex) : null;
    console.log(`[getHeroPoolForBucketWithExtra] extraRules:`, extraRules);

    if (extraRules) {
        console.log(`[getHeroPoolForBucketWithExtra] 使用 extra 规则构建池`);
        ...
        if (extraRules.includedFamilies && extraRules.includedFamilies.length > 0) {
            ...
            console.log(`[getHeroPoolForBucketWithExtra] 筛选 includedFamilies: ${families.join(', ')}`);

            // 先按星级初步筛选，便于调试
            const starFiltered = masterHeroPool.filter(h => h.star === targetStar);
            console.log(`[getHeroPoolForBucketWithExtra] 星级 ${targetStar} 的英雄总数: ${starFiltered.length}`);

            let candidates = starFiltered.filter(h => {
                const heroFamily = String(h.family || '').toLowerCase();
                const match = families.includes(heroFamily);
                if (!match) {
                    // 这里不输出所有不匹配的，只记录前几个，避免刷屏
                }
                return match;
            });
            console.log(`[getHeroPoolForBucketWithExtra] 家族匹配后: ${candidates.length}`);

            // 如果匹配数为0，输出前10个不匹配的英雄及其家族，方便排查
            if (candidates.length === 0) {
                console.warn(`[getHeroPoolForBucketWithExtra] 警告：家族匹配为0！目标家族: ${families.join(', ')}`);
                const sample = starFiltered.slice(0, 10).map(h => ({ id: h.heroId, family: h.family, name: h.name }));
                console.warn(`[getHeroPoolForBucketWithExtra] 前10个英雄的家族:`, sample);
            }

            candidates = applyCostumeRule(candidates, extraRules.includedCostume, poolConfig);
            console.log(`[getHeroPoolForBucketWithExtra] 应用服装规则后: ${candidates.length}`);
        ...
        if (extraRules.includedOrigins && extraRules.includedOrigins.length > 0) {
            const origins = extraRules.includedOrigins.map(o => {
                ...
                // 尝试通过 sourceReverseMap 映射，如果映射不存在则使用原字符串小写
                return sourceReverseMap[o] ? sourceReverseMap[o].toLowerCase() : lower;
            });
            console.log(`[getHeroPoolForBucketWithExtra] 筛选 includedOrigins: ${origins.join(', ')}`);
            ...
            let candidates = starFiltered.filter(h => {
                ...
                // 将英雄的 source 通过 sourceReverseMap 转为英文标识，若无则用小写原值
                const heroSourceEng = sourceReverseMap[h.source] ? sourceReverseMap[h.source].toLowerCase() : h.source.toLowerCase();
                const match = origins.includes(heroSourceEng);
                if (!match) {
                    // 可选记录
                }
                return match;
            });
            console.log(`[getHeroPoolForBucketWithExtra] 来源匹配后: ${candidates.length}`);

            if (candidates.length === 0) {
                console.warn(`[getHeroPoolForBucketWithExtra] 警告：来源匹配为0！目标来源: ${origins.join(', ')}`);
                const sample = starFiltered.slice(0, 10).map(h => ({ id: h.heroId, source: h.source, name: h.name }));
                console.warn(`[getHeroPoolForBucketWithExtra] 前10个英雄的来源:`, sample);
            }
        ...
        // 3. 处理 includeTrainers
        if (extraRules.includeTrainers) {
            console.log(`[getHeroPoolForBucketWithExtra] 添加训练师 (星级 ${targetStar})`);
            const trainers = createTrainerHeroes(targetStar);
            pool = pool.concat(trainers);
        }
        ...
        if (extraRules.oldestCharacterAgeInDays !== undefined || extraRules.youngestCharacterAgeInDays !== undefined) {
            ...
            if (baseDate) {
                console.log(`[getHeroPoolForBucketWithExtra] 应用年龄规则, baseDate: ${baseDate.toISOString()}`);
        ...
        console.log(`[getHeroPoolForBucketWithExtra] 无 extra 规则，回退到原 getHeroPoolForBucket`);
        const result = getHeroPoolForBucket(bucketString, { ...poolConfig, masterPool: masterHeroPool });
        console.log(`[getHeroPoolForBucketWithExtra] 原函数返回: ${result.length}`);
        return result;
    }
}
...
function getAllHeroesInPool(poolConfig) {
    ...
    // ★ 移除了 includedOrigins 顶层过滤，直接使用全部英雄 ★
    ...
    if (poolConfig.bucketConfig) {
        ...
        poolConfig.bucketConfig.forEach((bucketString, index) => {
            ...
            if (bucketString.startsWith('trainer')) {
                ...
                // ★ 调用新函数，传入 index 和 masterPoolForBuckets ★
                const heroesFromBucket = getHeroPoolForBucketWithExtra(bucketString, index, poolConfig, masterPoolForBuckets);
