...
function parseAndStoreDoTInfo(hero) {
    ...
    hero.effects.forEach((effectText, index) => {
        ...
        // ========== 多伤害专属规则 ==========
        const mulDamages = (effectText.includes('共振') || effectText.includes('Resonance') || effectText.includes('Wild') || effectText.includes('荒野：') || effectText.includes('所受伤害') || effectText.includes('诅咒伤害') || effectText.includes('詛咒傷害')) && (effectText.includes('敌人') || effectText.includes('敵人') || effectText.includes('damage'));
        if (mulDamages) {
